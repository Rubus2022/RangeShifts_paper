rShift_sim<-function(r=1){
  ...
  nest_fun<-function(net){
    if(ecount(net)>0){
      if(is.weighted(net)==T){
        return(NODF(import.RInSp(get.adjacency(net,sparse=F,attr="weight")))$NODF)
      } else {NODF(import.RInSp(get.adjacency(net,sparse=F)))$NODF}
    } else {return(0)}
  }
  ...
  process_nets<-function(Initial,Final,Ints,trophic=F, interactions=T,cut_value=0.5){
    ...
    #regional networks####
    regWeb_pre<-metaweb(nets_pre[51:100][sapply(51:100,function(x){length(E(nets_pre[x][[1]]))>0})])
    if(is.weighted(regWeb_pre)){
    regWeb_pre<-delete_edge_attr(regWeb_pre,"weight")}
    
    regWeb_post<-metaweb(nets_post[51:100][sapply(51:100,function(x){length(E(nets_post[x][[1]]))>0})])
    if(is.weighted(regWeb_post)){
    regWeb_post<-delete_edge_attr(regWeb_post,"weight")}
    ...
    rcWeb_pre<-metaweb(nets_pre[1:50][sapply(1:50,function(x){length(E(nets_pre[x][[1]]))>0})])
    if(is.weighted(rcWeb_pre)){
      rcWeb_pre<-delete_edge_attr(rcWeb_pre,"weight")}
    
    rcWeb_post<-metaweb(nets_post[51:100][sapply(51:100,function(x){length(E(nets_post[x][[1]]))>0})])
    if(is.weighted(rcWeb_post)){
      rcWeb_post<-delete_edge_attr(rcWeb_post,"weight")}
    ...
    link_dis_bray<-(as.matrix(vegdist(t(nets_bin),method = "bray",binary = F))[51:150,151:200])
    min_dist<-apply(link_dis_bray,2,function(x){min(which(x==min(x)))})
    link_dis_bray<-(as.matrix(vegdist(t(nets_bin),method = "bray",binary = F))[51:150,151:200])
    bray_beta_Ln<-mean(apply(link_dis_bray,2,function(x){x[min(which(x==min(x)))]}))
    ...
    betaLink_Ln$braycurt<-c(bray_beta_Ln,NA,NA)
    names(betaLink_Ln)<-c("S","OS","WN","ST","BC")
    betaLink_Ln$Part<-c("All","Gain","Loss")
    
    #net indicies
    netInd_Ln<-colMeans(unnest(data.frame(t(sapply(51:100,function(x){
      if(is.weighted(nets_post[[x]])){GenInd2(get.adjacency(nets_post[[x]],sparse = F,attr="weight"))}
      else {GenInd2(get.adjacency(nets_post[[x]],sparse = F))}
    }))))/ #this is where the issue is - networks without species have no weights
      unnest(data.frame(t(sapply(min_dist,function(x){
        if(is.weighted(nets_pre[[x]])){
          GenInd2(get.adjacency(nets_pre[[x]],sparse = F,attr="weight"))
        } else {GenInd2(get.adjacency(nets_pre[[x]],sparse = F))}
      })))),na.rm=T)
    ...
    #local patches####
    netInd_Lp<-colMeans(unnest(data.frame(t(sapply(51:100,function(x){if(is.weighted(nets_post[[x]])){GenInd2(get.adjacency(nets_post[[x]],sparse = F,attr="weight"))
    }else{GenInd2(get.adjacency(nets_post[[x]],sparse = F))}}))))/
      unnest(data.frame(t(sapply(51:100,function(x){if(is.weighted(nets_pre[[x]])){GenInd2(get.adjacency(nets_pre[[x]],sparse = F,attr="weight"))
      }else{GenInd2(get.adjacency(nets_pre[[x]],sparse = F))}})))),na.rm=T)
    ...
    #combine results
    betaLink<-rbind.data.frame(betaLink_R,betaLink_cR,betaLink_Ln)
    ...
    if(trophic==T){
      ...
      Trophic_BL<-rbind.data.frame(regBL,rcBL,LnBL)
      Trophic_BL$Rep<-r
  ...
  TN_process<-function(pre,post){
    ...
    if(is.weighted(plant_pre)==T){
      Troph_types<-c("plant","herb","pred")
      Troph_BC<-rep(NA,3)
      for(j in 1:length(Troph_types)){
        pre_adj<-as_adj(get(paste(Troph_types[j],"_pre",sep="")),attr="weight",sparse=F)
        if(length(pre_adj)==0){
          Troph_BC[j]<-NA
        } else {
          pre.df<-data.frame(Ints=c(pre_adj),from=rep(colnames(pre_adj),each=nrow(pre_adj)),to=rownames(pre_adj),time="pre")
          
          post_adj_net<-get(paste(Troph_types[j],"_post",sep=""))
          if(length(E(post_adj_net))==0){
            Troph_BC[j]<-1
          } else {
            post_adj<-as_adj(get(paste(Troph_types[j],"_post",sep="")),attr="weight",sparse=F)
            if(length(post_adj)==0){
              Troph_BC[j]<-1
            } else {
              post.df<-data.frame(Ints=c(post_adj),from=rep(colnames(post_adj),each=nrow(post_adj)),to=rownames(post_adj),time="post")
              pre_post.df<-rbind(pre.df,post.df)
              pre_post_com<-pre_post.df %>% 
                filter(Ints!=0) %>% 
                mutate(Int_name=paste(from,to,sep=" ")) %>% 
                select_("Ints","Int_name","time") %>% 
                spread(key = Int_name,Ints,fill = 0)
              Troph_BC[j]<-vegdist(pre_post_com[,-1],method = "bray",binary = F)
            }
          }
        }
      }
      beta_troph$BC<-c(Troph_BC,rep(NA,6))
    } else {
      beta_troph$BC<-NA
    }
  ...
  dispV<-c(0.0001,0.0005,0.001,0.005,0.01,0.05,0.1,0.5)
  ...
  ddV<-rnorm(n = n,mean = dd[1],sd=(dd[1]*0.25))
  ddV[ddV<0]<-0
  
  Disp_array<-array(NA,dim=c(nCom,nCom,n))
  for(s in 1:n){
    Disp<-exp(-ddV[s]*Disp2)-diag(nrow(Disp2))
    ...
    Disp<-decostand(Disp,"total",2)
    Disp_array[,,s]<-Disp
  }
  #matplot(Disp_array[1,,],type='l')
  ...
  C3<-c(rep(0.1,nprey),rep(-0.001,n-nprey))
  
  #Dispersal kernels for Herbivores
  ddV<-rnorm(npred1,dd[2],sd=dd[2]*0.25)
  ddV[ddV<0]<-0
  
  Disp_array_h<-array(NA,dim=c(nCom,nCom,npred1))
  for(s in 1:npred1){
    Disp<-exp(-ddV[s]*Disp2)-diag(nrow(Disp2))
    Disp<-Disp[((nCom/2)+1):((nCom/2)+nCom),((nCom/2)+1):((nCom/2)+nCom)]+rbind(apply(Disp[1:(nCom/2),((nCom/2)+1):((nCom/2)+nCom)],2,rev),apply(Disp[((nCom/2)+nCom+1):(nCom*2),((nCom/2)+1):((nCom/2)+nCom)],2,rev))
    Disp<-decostand(Disp,"total",2)
  ...
  #Dispersal kernels for predators
  ddV<-rnorm(npred2,dd[3],sd=dd[3]*0.25)
  ddV[ddV<0]<-0
  
  Disp_array_c<-array(NA,dim=c(nCom,nCom,npred2))
  for(s in 1:npred2){
    Disp<-exp(-ddV[s]*Disp2)-diag(nrow(Disp2))
    Disp<-Disp[((nCom/2)+1):((nCom/2)+nCom),((nCom/2)+1):((nCom/2)+nCom)]+rbind(apply(Disp[1:(nCom/2),((nCom/2)+1):((nCom/2)+nCom)],2,rev),apply(Disp[((nCom/2)+nCom+1):(nCom*2),((nCom/2)+1):((nCom/2)+nCom)],2,rev))
    Disp<-decostand(Disp,"total",2)
  ...
  for(d in 1:length(dispV)){
    disp<-dispV[d]
    disp1<-rnorm(n,mean = disp,sd = (0.25*disp))
    disp1[disp1<0]<-0
    disp1[disp1>1]<-1
    ...
    for(l in 1:(length(StressV)-1)){
      #print(l)
      Xt<-X*exp(rep(C,nCom)+BN%*%X+t(A[(ComStart+Stress[StressV[l]]),]))
      Xt<-Xt+do.call(rbind,lapply(1:n,function(j) Xt[j,]%*%Disp_array[,,j]))*disp1-disp1*Xt
      Xt[Xt<10^-3]<-0
      X<-Xt
      
      XIt<-XI*exp(rep(C,nCom)+BI%*%XI+t(A[(ComStart+Stress[StressV[l]]),]))
      XIt<-XIt+do.call(rbind,lapply(1:n,function(j) XIt[j,]%*%Disp_array[,,j]))*disp1-disp1*XIt
      XIt[XIt<10^-3]<-0
      XI<-XIt
      
      XMt<-XM*exp(rep(C,nCom)+BM%*%XM+t(A[(ComStart+Stress[StressV[l]]),]))
      XMt<-XMt+do.call(rbind,lapply(1:n,function(j) XMt[j,]%*%Disp_array[,,j]))*disp1-disp1*XMt
      XMt[!is.finite(XMt)]<-0
      XMt[(XMt<10^-3)]<-0
      XM<-XMt
      
      X3t[preyV,]<-X3[preyV,]*exp(rep(C3[preyV],nCom)+B3[preyV,]%*%X3+t(A3[(ComStart+Stress[StressV[l]]),preyV]))
      X3t[preyV,]<-X3t[preyV,]+do.call(rbind,lapply(1:nprey,function(j) X3t[preyV,][j,]%*%Disp_array[,,j]))*disp1-disp1*X3t[preyV,]
      
      X3t[pred1V,]<-X3[pred1V,]*exp(rep(C3[pred1V],nCom)+B3[pred1V,]%*%X3+t(A3[(ComStart+Stress[StressV[l]]),pred1V]))
      X3t[pred1V,]<-X3t[pred1V,]+do.call(rbind,lapply(1:npred1,function(j) X3t[pred1V,][j,]%*%Disp_array_h[,,j]))*disp1-disp1*X3t[pred1V,]
      
      X3t[pred2V,]<-X3[pred2V,]*exp(rep(C3[pred2V],nCom)+B3[pred2V,]%*%X3+t(A3[(ComStart+Stress[StressV[l]]),pred2V]))
      X3t[pred2V,]<-X3t[pred2V,]+do.call(rbind,lapply(1:npred2,function(j) X3t[pred2V,][j,]%*%Disp_array_c[,,j]))*disp1-disp1*X3t[pred2V,]
